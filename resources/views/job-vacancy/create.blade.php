            ...
            <form action="{{ route('job-vacancies.store') }}" method="POST">
                ...
                <div class="mb-4 p-6 bg-gray-50 border border-gray-100 rounded-lg shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" class=" {{ $errors->has('title') ? 'border border-red-500' : 'border border-gray-300' }} rounded-md p-2 w-full" >
                        @error('title')
                    ...
                    <div class="mb-4 w-full" >
                        ...
                        <textarea rows="3" type="text" name="description" value="{{ old('description') }}"
                    ...
                    <div class="mb-2">
                        ...
                        <input type="text" name="location" value="{{ old('location') }}" class=" {{ $errors->has('location') ? 'border border-red-500' : 'border border-gray-300' }} rounded-md p-2 w-full" >
                        @error('location')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <input type="number" name="salary" value="{{ old('salary') }}" class=" {{ $errors->has('salary') ? 'border border-red-500' : 'border border-gray-300' }} rounded-md p-2 w-full" >
                        @error('salary')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <select id="jobVacancyType" name="jobVacancyType" class=" {{ $errors->has('jobVacancyType') ? 'border border-red-500' : 'border border-gray-300' }} rounded-md p-2 w-full" >
                            @foreach ( $jobVacanciesTypes as $jobVacancyType )
                                <option value="{{ $jobVacancyType }}" {{ old('jobVacancyType') == $jobVacancyType ? 'selected' : '' }}> {{ $jobVacancyType }} </option>
                            @endforeach
                        </select>
                        @error('type')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
